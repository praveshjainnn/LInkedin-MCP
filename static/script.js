document.addEventListener('DOMContentLoaded', () => {
    ...
    const setKeyBtn = document.getElementById('set-groq-key-btn');
    ...
    setKeyBtn.addEventListener('click', () => {
        if (groqKeyInput.value.length > 5) {
            ...
            setKeyBtn.classList.add('active-key');
            setTimeout(() => {
                setKeyBtn.textContent = 'SET KEY';
                setKeyBtn.classList.remove('active-key');
            }, 3000);
        } else {
            setKeyBtn.textContent = 'ERR';
            setTimeout(() => setKeyBtn.textContent = 'SET KEY', 1500);
        }
    });
    ...
    genImagesBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            // Style config
            const styleConfig = {
                '3d render':        { color: '#810CA8', emoji: '' },
                '3d':               { color: '#810CA8', emoji: '' },
                'cinematic':        { color: '#b45309', emoji: '' },
                'cinematic photo':  { color: '#b45309', emoji: '' },
                'flat illustration':{ color: '#C147E9', emoji: '' },
                'illustration':     { color: '#C147E9', emoji: '' },
                'minimal':          { color: '#E5B8F4', emoji: '' },
            };
...
class ParticleSystem {
    constructor() {
        ...
        this.colors = ['#2D033B', '#810CA8', '#C147E9', '#E5B8F4', '#9d4edd', '#c77dff'];
