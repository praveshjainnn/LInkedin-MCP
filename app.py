...
"""
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            llm_provider.change(
                fn=lambda x: gr.update(visible=(x == "groq")),
                inputs=llm_provider,
                outputs=grok_api_key,
            )
    ...
    def ui_wrapper(brand_desc, sample_posts, pillar_text, use_trending_news, feed_url, n_posts, llm_provider, api_key):
        md_text, csv_path = run_linkedin_agent(
            brand_desc, sample_posts, pillar_text, n_posts, use_trending_news, feed_url, llm_provider, api_key
        )
        if csv_path:
    ...
    run_btn.click(
        ui_wrapper,
        inputs=[brand_desc, sample_posts, pillar_text, use_trending_news, feed_url, n_posts, llm_provider, grok_api_key],
